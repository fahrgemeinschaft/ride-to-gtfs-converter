...
public class ApplicationPropertiesValidator {
	...
	public void validArea() {
		if (constraints.area == null || constraints.area.trim().isEmpty()) {
	...
	public boolean validMailAddresses(String[] recipients) {
		...
		if (!mailSenderAddress.trim().matches(VALID_EMAIL_ADDRESS)) {
			LOG.error("Sender for GTFS validation alerts has an invalid e-mail address: " + mailSenderAddress);
			return false;
		}
		for (String recipient : recipients) {
			if (!recipient.matches(VALID_EMAIL_ADDRESS)) {
				LOG.error("One recipient for GTFS validation alerts has an invalid e-mail address: " + recipient);
				return false;
			}
		}
		return true;
